...
LinkedIn maximum limit: 3000 characters.

The FINAL post must stay under 2700 characters TOTAL.
...
You are generating only the main content, but CTA, emoji, and hashtags will be added later.

So you MUST leave enough space for them.

Write concise, controlled, and avoid unnecessary length.

If unsure about length, write shorter.
...
- Follow the requested format structure
- Keep paragraphs tight and readable
- Do NOT include hashtags in body
- End naturally with the CTA
- Prefer depth and insight instead of long text

Generate only the post content within the length limit.
""".strip()
...
# ============================================================
# LENGTH SAFETY GUARD (BULLETPROOF)
# ============================================================

def enforce_linkedin_limit(text: str, limit: int = 3000, max_attempts: int = 3):
    ...
    for attempt in range(max_attempts):
        ...
        reduction_prompt = f"""
The following LinkedIn post exceeds platform limits.

Rewrite it shorter while keeping:
...
The final version MUST be under {limit - 200} characters.

POST:
{current_text}
"""

        shorter = generate_text(reduction_prompt).strip()
    ...
    # Final fallback — return last version even if slightly over
    # (extremely rare after retries)
    return current_text
...
""".strip()
    ...
    final_post = enforce_linkedin_limit(final_post, 2900)
